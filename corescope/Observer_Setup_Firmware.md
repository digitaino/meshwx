...
Meshcore broker over WiFi. **No host computer needed.**
...
to build and flash it first. Come back here for configuration.
...
- A flashed radio (see the upstream guide above).
- A USB cable to the radio.
- A computer with a serial-console terminal (`screen`, `picocom`,
...
## 1. Connect to the radio over serial

Plug the radio into your computer via USB. The radio enumerates as a
USB-serial device.
...
You should see firmware log lines streaming. Hit Enter to get a
command prompt.
...
After the reboot finishes (~5 seconds), reconnect to the serial
console. You should see lines indicating the radio is joining WiFi
and then connecting to MQTT. If you missed them, query state:
...
- This firmware fork is a branch of the upstream MeshCore project.
  ...
  The radio can still be a regular Meshcore node (companion,
  repeater, room) per the firmware's docs — it just gains
  MQTT-publishing as a side capability.
