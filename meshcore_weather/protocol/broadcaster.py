...
class AppResponder:
    ...
    async def handle_request(self, text: str, sender_key: str) -> str:
        ...
        seq, bot = self._scheduler.seq, self._scheduler.bot_id()
        try:
    ...
    def _answer(self, cmd: str, arg: str, seq: b.SeqCounter, bot: int) -> list[bytes]:
        ...
        if cmd == "f":
            ...
            if lat is None:
                return [b.not_available(seq.next(), bot, "f", v5.REASON_UNKNOWN_LOCATION)]
            msg = b.forecast_message(seq.next(), bot, self.store, float(lat), float(lon))
            return [msg] if msg else []
        ...
        if cmd in ("hwo", "taf"):
            ...
            taf = services.taf_for(self.store, loc)
            text = getattr(taf, "text", None) or getattr(taf, "raw", None) if taf else None
            return b.text_messages(seq, bot, v5.SUBJECT_METAR, text) if text else []

        if cmd in ("space", "storm", "rain", "metar"):
            subject = {"space": v5.SUBJECT_SPACE, "storm": v5.SUBJECT_STORM_REPORTS,
                       "rain": v5.SUBJECT_RAINFALL, "metar": v5.SUBJECT_METAR}[cmd]
